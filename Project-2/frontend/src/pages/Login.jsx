...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const studentId = decoded.studentId; // Extract studentId from the decoded token
  ...
  return (
    <div className="relative min-h-screen bg-teal-300 overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-cover bg-fixed opacity-30" style={{ backgroundImage: "url('https://via.placeholder.com/1500x1000.png?text=Student+Portal')" }}></div>

      {/* Main Content */}
      <div className="relative z-10 flex items-center justify-center min-h-screen text-center text-white px-6 py-12">
        <div className="max-w-sm w-full p-8 bg-green-500 border border-gray-300 rounded-xl shadow-2xl z-20">
          <h2 className="text-4xl font-extrabold text-center text-white mb-6">Student Login</h2>
          <p className="text-lg text-gray-200 mb-6">
            Please enter your credentials to access the student portal.
          </p>
          <form onSubmit={handleLogin}>
            ...
            <button
              type="submit"
              className="w-full p-4 bg-blue-700 text-white rounded-md hover:bg-blue-800 transition-all duration-300"
            >
              Login
            </button>
          </form>
